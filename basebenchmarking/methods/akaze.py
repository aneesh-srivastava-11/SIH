"""
AKAZE Image Registration Baseline Method.
"""
...
class AKAZEMethod(RegistrationMethod):
    """Off-the-shelf AKAZE Baseline Adapter (Binary Descriptors + Hamming Distance)."""
    ...
    def is_available(cls) -> Tuple[bool, str]:
        try:
            ...
            return False, "OpenCV AKAZE_create returned None"
        except AttributeError:
            return False, "OpenCV installation does not include AKAZE"
        except Exception as e:
            return False, f"AKAZE initialization failed: {e}"
    ...
    def run(self, reference_image: np.ndarray, target_image: np.ndarray, config: Any, pair_id: str = "") -> RegistrationResult:
        ...
        if des1 is None or des2 is None or len(kp1) < min_matches or len(kp2) < min_matches:
            ...
            return RegistrationResult(
                ...
                error_message=f"Insufficient keypoints detected (ref: {num_kp_ref}, tgt: {num_kp_tgt}, min: {min_matches})",
            )

        # BFMatcher with Hamming distance for AKAZE binary descriptors
        ...
        if num_good < min_matches:
            ...
            return RegistrationResult(
                ...
                error_message=f"Too few good matches after ratio test ({num_good} < {min_matches})",
            )
